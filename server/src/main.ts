...
const connection: server.Connection = typeof process.send === "function"
...
connection.onInitialize(params => {
    ...
    if (foldingRange && "lineFoldingOnly" in foldingRange && foldingRange.lineFoldingOnly !== undefined)
...
function parseWorkspace(): void {
    ...
    function appendFile(completePath: string, cMode: boolean): void {
        if (nOpenend < 1000) {
            ...
            nOpenend++;
            pp.parseFile(completePath, fileUri, cMode).then(
                prov => {
                    nOpenend--;
                    UpdateFile(prov);
        ...
        fs.readdir(dir, function (_err, ff) {
            ...
            for (let i = 0; i < ff.length; i++) {
                ...
                if (info.isFile()) {
                    ...
                    cFile = !prgFile && (pathParse.ext.startsWith(".c") || pathParse.ext === ".h");
                } else {
                    cFile = false;
                    prgFile = false;
                }
                files[i] = { name: ff[i], completePath, info, pathParse, prgFile, cFile };
            }
...
function AddIncludes(startPath: string, includesArray: string[]): void {
    ...
    function FindInclude(dir: string, fileName: string): boolean {
        ...
        pp.parseFile(completePath, canonicalFileUri, false).then(
            prov => {
...
function ParseInclude(startPath: string, includeName: string, addGlobal: boolean): provider.Provider | undefined {
    ...
    function FindInclude(dir: string): provider.Provider | undefined {
        ...
        pp.parseString(fs.readFileSync(test).toString(), canonicalUri(Uri.file(test).toString()));
        if (addGlobal)
...
function kindToVS(kind: string, sk?: boolean): server.SymbolKind | server.CompletionItemKind {
...
connection.onDocumentSymbol((param) => {
    ...
    for (let fn = 0; fn < p.funcList.length; fn++) {
        ...
        if (info.parent && info.parent.endLine !== undefined && info.startLine <= info.parent.endLine) {
            let pp: provider.Info | undefined = info.parent;
            const names: string[] = [];
            while (pp) {
                if (pp.kind === "method" && pp.foundLike === "definition" && (!pp.parent || (pp.parent.endLine !== undefined && pp.startLine > pp.parent.endLine))) {
                    if (pp.parent)
    ...
    for (const file in files) {
        ...
        for (let fn = 0; fn < pp.funcList.length; fn++) {
            ...
            dest.push(server.SymbolInformation.create(
                info.name, kindToVS(info.kind) as server.SymbolKind,
                server.Range.create(info.startLine, info.startCol,
                    info.endLine ?? info.startLine, info.endCol),
...
function GetWord(params: server.TextDocumentPositionParams, withPrev?: boolean): GetWordResult {
    ...
    while (true) {
        ...
        while ((candidate = r.exec(text))) {
            if (candidate.index <= txtPos && candidate.index + candidate[0].length >= txtPos) {
                word = candidate;
                break;
            }
        }
...
connection.onDefinition((params): server.Definition | server.LocationLink[] | undefined => {
    const doc = documents.get(params.textDocument.uri);
    ...
    if (include !== null) {
        ...
        const found = definitionFiles(include[1], startPath, server.Range.create(params.position.line, pos, params.position.line, pos + include[1].length));
        return canLocationLink ? found as server.LocationLink[] : found as server.Location[];
    }
    ...
    function DoProvider(pp: provider.Provider, file: string): void {
        for (let fn = 0; fn < pp.funcList.length; fn++) {
            ...
            if (info.nameCmp !== word)
                continue;
            if (info.kind.endsWith("*") && file !== docUri)
                continue;
            if (info.kind === 'static' && file !== docUri)
                continue;
            ...
            if (info.kind === 'local' || info.kind === 'param') {
                ...
                if (ppParent) {
                    if (ppParent.startLine > params.position.line)
                        continue;
                    if (ppParent.endLine !== undefined && ppParent.endLine < params.position.line)
                        continue;
                }
            }
    ...
    while (i < incList.length) {
        ...
        if (pp) {
            ...
            for (let j = 0; j < pp.includes.length; j++) {
                if (incList.indexOf(pp.includes[j]) < 0)
                    incList.push(pp.includes[j]);
            }
        }
        i++;
    }

    return canLocationLink ? dest as server.LocationLink[] : dest as server.Location[];
});
...
connection.onSignatureHelp((params) => {
    ...
    let signatures: server.SignatureInformation[] = ([] as server.SignatureInformation[]).concat(getWorkspaceSignatures(word, doc, className, nC));
    if (signatures.length === 0 && className !== undefined) {
        signatures = ([] as server.SignatureInformation[]).concat(getWorkspaceSignatures(word, doc, undefined, nC));
    }
    signatures = signatures.concat(getStdHelp(word, nC));
    return { signatures, activeParameter: nC };
});

function findBracket(text: string, pos: number, dir: number, bracket: string): number | undefined {
    let nP = 0;
...
function getWorkspaceSignatures(
    ...
    let thisDone = false;
    function GetSignatureFromInfo(pp: provider.Provider, info: provider.Info, iSign: number): server.SignatureInformation | undefined {
        if (info.hDocIdx !== undefined) return GetHelpFromDoc(pp.harbourDocs[info.hDocIdx]);
    ...
    for (const file in files) {
        ...
        for (let iSign = 0; iSign < pp.funcList.length; iSign++) {
            ...
            if (s && s.parameters && s.parameters.length >= nC)
                signatures.push(s);
        }
    }
    if (!thisDone) {
...
function GetHelpFromDoc(doc: provider.HarbourDocInfo): server.SignatureInformation {
    const s: server.SignatureInformation = { label: doc.label ?? "", parameters: [] };
...
let lastDocOutsideWorkspaceProvider: provider.Provider = new provider.Provider();
function getDocumentProvider(doc: server_textdocument.TextDocument, checkGroup?: boolean): provider.Provider {
    const docUri = canonicalUri(doc.uri);
...
connection.onCompletion((param, cancelled) => {
    ...
    function CheckAdd(label: string, kind: server.CompletionItemKind, sort: string): server.CompletionItem | undefined {
    ...
    function GetCompletions(pp: provider.Provider, file: string): server.CompletionList | undefined {
        for (let iSign = 0; iSign < pp.funcList.length; iSign++) {
            const info = pp.funcList[iSign];
            if (word.length > 0 && !IsInside(word, info.nameCmp))
                continue;
            if (info.endCol === param.position.character && info.endLine === param.position.line && file === docUri)
                continue;
            ...
            if (info.parent && (info.parent.kind.startsWith("function") || info.parent.kind.startsWith("procedure") || info.parent.kind === 'method')) {
                ...
                if (param.position.line < info.parent.startLine ||
                    (info.parent.endLine !== undefined && param.position.line > info.parent.endLine))
                    continue;
            }
            const added = CheckAdd(info.name, kindToVS(info.kind, false) as server.CompletionItemKind, "AAA");
    ...
    if (prevLetter !== ':' && prevLetter !== '->') {
        for (let i = 0; i < docs.length; i++) {
            ...
            const c = CheckAdd(docs[i].name!, server.CompletionItemKind.Function, "AA");
    ...
    if (wordBasedSuggestions && !pp) {
        ...
        while ((foundWord = wordRE.exec(line))) {
            // remove current word
            if (foundWord.index < cursor && foundWord.index + foundWord[0].length >= cursor)
                continue;
...
function CompletionDBFields(word: string, allText: string, pos: number, pp: provider.Provider | undefined): server.CompletionItem[] {
    let pdb = pos - 2;
    let dbName = "";
    let nBracket = 0;
    ...
    function CheckDB(databasesRec: Record<string, provider.DbInfo | AggregatedDbInfo>): void {
...
connection.onHover((params, cancelled) => {
    ...
    if (pp) {
        const result = pp.funcList.filter((v) => v.kind === 'define' && v.name === word);
        if (result.length > 0) {
    ...
    if (pp) {
        ...
        while (i < incList.length) {
            const pInc = ParseInclude(startDir, incList[i], thisDone);
            if (pInc) {
                const result = pInc.funcList.filter((v) => v.kind === 'define' && v.name === word);
                if (result.length > 0) {
                    return { contents: { language: 'harbour', value: result[0].body ?? "" } };
                }
                for (let j = 0; j < pInc.includes.length; j++) {
    ...
    function buildHoverForInfo(info: provider.Info, pp: provider.Provider): server.Hover | undefined {
        let label = "";
        switch (info.kind) {
            case "function":
            case "function*":
        ...
        for (let i = 0; i < pp.funcList.length; i++) {
            ...
            if (info.kind === "local" || info.kind === "param") {
                if (info.parent) {
                    ...
                    if (info.parent.endLine !== undefined && info.parent.endLine < params.position.line) continue;
                }
            }
...
connection.onRequest(server.SemanticTokensRequest.method, (param: server.SemanticTokensParams) => {
    const doc = documents.get(param.textDocument.uri);
    if (!doc) return { data: [] };
    const docUri = canonicalUri(doc.uri);
    let ret: number[][] = [];
...
connection.onReferences((params) => {
    ...
    const def = pThis.funcList.find((v) =>
        v.nameCmp === word &&
        (v.parent === undefined || (v.parent.startLine <= reqLine && (v.parent.endLine === undefined || v.parent.endLine >= reqLine))));
    let onlyThis = false;
    if (def) {
        kind = def.kind;
    ...
    if (word in pThis.references) {
        for (let i = 0; i < pThis.references[word].length; i++) {
            ...
            if (def && def.parent && onlyThis) {
                ...
                if (def.parent.endLine !== undefined && ref.line > def.parent.endLine) continue;
            }
            ret.push(server.Location.create(docUri,
                server.Range.create(ref.line, ref.col, ref.line, ref.col + word.length)));
        }
    }

    if (!onlyThis) for (const file in files) {
        if (file === docUri) continue;
        const pp = files[file];
...
function getNextNotSpace(doc: server_textdocument.TextDocument, startPos: number): string {
 ...
 */
function getCleanline(_line: string, lineState: provider.lineState | undefined, precLineState: provider.lineState | undefined): string {
    let line = _line;
    let i = 0;
    ...
    for (let i = 0; i < pThis.groups.length; ++i) {
        ...
        if (currStyleConfig?.indent?.switch && currStyleConfig?.indent?.case &&
            ...
            for (let p = 1; p < group.positions.length; ++p) {
                ...
                if (typeof text === "string" ? text.startsWith("case") : Array.isArray(text) ? typeof text[0] === "string" && text[0].startsWith("case") : false)
                    tabs[group.positions[p].line] -= 1;
            }
        }
    }
