...
export const keywords: string[] = ["local", "static", "private", "memvar",
...
export type ReferenceType = "variable" | "function" | "data" | "method" | "field";
...
export class reference {
    ...
    col: number;
    howWrite: string;
    constructor(type: ReferenceType, line: number, col: number, howWrite: string) {
        this.type = type;
        this.line = line;
        this.col = col;
...
export class Group {
    ...
    addRange(line: number, startCol: number, endCol: number, text: unknown): void {
...
export class Provider {
        ...
        if (search) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (m) {
                    ...
                    const ii = new Info(name, kind, like, parent, this.currentDocument,
    ...
    parseDeclareList(list: string, kind: string, parent: Info | string | undefined): void {
        let i = -1;
        while (true) {
            i++;
    ...
    parseCommand(translate: boolean): void {
        ...
        const headerMatch = this.currLine.match(/^\s*#\w?(?:command|translate)\s+/i);
        ...
        for (let j = 0; j < commandResult.length; ++j) {
            ...
            const snip = CommandPartToSnippet(commandResult[j].text, commandResult[j].fixed, resultPart);
            if (!snip) return;
        ...
        while (!commandResult.regEx) {
            ...
            if (k > 0 || translate)
                commandResult.regEx = commandResult[k].regEx;
            else
                commandResult.regEx = new RegExp("^\\s*" + commandResult[k].regEx!.source, "i");
        }
    ...
    parseHarbour(words: string[]): void {
        ...
        if (words[0][0] === '#') {
            if (words[0] === '#include') {
                ...
                const incWords = this.currLinePreProc.replace(/\s+/g, " ").trim().split(" ");
            ...
            if (this.currentClass && (words[0] === "endclass" || (words[0] === "end" && words[1] === "class"))) {
                ...
                if ((words[0] === "class" && (!this.currentClass || this.currentClass.endLine)) || (words[0] === "create" && words[1] === "class")) {
                    ...
                    this.currentMethod = undefined;
                    if (words[0] === "create")
                        this.currentClass = this.addInfo(words[2], 'class', "definition", undefined);
                    else
                        this.currentClass = this.addInfo(words1, 'class', "definition", undefined);
                    this.currentClass.endLine = undefined;
                } else if (["data", "var", "classdata", "classvar"].indexOf(words[0]) >= 0 ||
                ...
                } else if (words[0] === "method" || words[0] === "classmethod" || (words[0] === "class" && words[1] === "method")) {
                ...
                } else if (words[0] === "procedure".substring(0, words[0].length) ||
                    ...
                    if (r) {
                        let kind = r[1].startsWith('p') || r[1].startsWith('P') ? "procedure" : "function";
                        if (words[0].startsWith("stat")) kind += "*";
                        if (this.currentMethod) this.currentMethod.endLine = this.lastCodeLine;
                        this.currentMethod = this.addInfo(r[2], kind, "definition", undefined);
                        if (r[3] && r[3].length)
                            this.parseDeclareList(r[3], "param", this.currentMethod);
                    }
                } else if (words[0] === "local".substring(0, words[0].length) ||
     ...
     * @returns this
     */
    parseFile(file: string, docName: string, cMode?: boolean, encoding?: BufferEncoding): Promise<Provider> {
        this.Clear();
        if (cMode !== undefined)
            this.cMode = cMode;
        ...
        return new Promise<Provider>((resolve) => {
            const reader = readline.createInterface({ input: fs.createReadStream(file, enc) });
...
function CommandPartToSnippet(text: string, fixed: boolean, resultPart: string): { snippet: string; repeatable: boolean } | undefined {
    let snippet = text;
