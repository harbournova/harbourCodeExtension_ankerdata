...
class HRBTask implements vscode.TaskProvider {
    ...
    resolveTask(task: vscode.Task, _token: vscode.CancellationToken): vscode.Task | undefined {
        const definition = task.definition as unknown as { input: string; output?: string; "c-type"?: string };
...
function getTerminalFn(task: vscode.Task): () => HBMK2Terminal {
    ...
    return () => {
        if (!myTerminals[task.name]) myTerminals[task.name] = new HBMK2Terminal(task);
        const taskBatch = getBatch(task);
        const existing = myTerminals[task.name];
        if (existing && (existing.batch || taskBatch) && taskBatch !== existing.batch) {
...
function getBatch(task: vscode.Task): string | undefined {
    ...
    if (platform in def) {
        const platformSpecific = def[platform] as { env?: Record<string, string>; setupBatch?: string };
...
class HBMK2Terminal implements vscode.Pseudoterminal {
    ...
    start(): void {
        ...
        const def = task.definition as unknown as TaskDefinitionLike;
        const inputFile = ToAbsolute(resolvePredefinedVariables(def.input)) || def.input;
        const section = vscode.workspace.getConfiguration("harbour");
        const warningLevel = section.get<number>("warningLevel", 1);
        const compilerExecutable = section.get<string>("compilerExecutable", "harbour");
        ...
        const tc = this;
        this.p.stderr.on("data", (data: Buffer | string) => tc.write(data.toString()));
        this.p.stdout.on("data", (data: Buffer | string) => tc.write(data.toString()));
        this.p.on("close", (r) => {
            tc.p = undefined;
...
export class HBMK2Task implements vscode.TaskProvider {
    getValidTask(
        ...
        retTask.execution = new vscode.CustomExecution(async () => getTerminalFn(retTask)());
    ...
    provideTasks(token: vscode.CancellationToken): vscode.ProviderResult<vscode.Task[]> {
        ...
        return new Promise<vscode.Task[]>((resolve, reject) => {
            ...
            if (textDocument && textDocument.languageId === "harbour") {
                ...
                task.execution = new vscode.CustomExecution(async () => getTerminalFn(task)());
                ...
                task2.execution = new vscode.CustomExecution(async () => getTerminalFn(task2)());
